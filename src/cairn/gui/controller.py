...
class CAIRNController(QObject):
    ...
    def _build_trainer(self):
        ...
        dataset = create_demo_dataset("data/sample", window_size=30, stride=10)
        if len(dataset) == 0:
        ...
        if not self._modules.ssm_branch:
            enc = model.state_builder.metric_enc
            if hasattr(enc, "use_ssm"):
                enc.use_ssm = False  # type: ignore[assignment, misc]
    ...
    def start_analysis(self) -> None:
        ...
        worker = AnalysisWorker(self, parent=self)
    ...
    def _run_analysis_core(self) -> list[dict]:
        ...
        funnel   = CascadeFunnel(l0_top_k=5, l1_top_k=3, l2_top_k=1)
        ...
        result = ALPVerifier(anomaly_threshold=0.0, ce_threshold=0.0).verify(chain, text)
        ...
        return [
            {
                ...
                "fault_type": "cpu_exhaustion" if i == 0 else "—",
